...
from pr_action.algo.utils import Range, process_description
...
class GitProvider(ABC):
    ...
    def get_pr_description(self, full: bool = True, split_changes_walkthrough=False) -> str or tuple:
        from pr_action.config_loader import get_settings
        from pr_action.algo.utils import clip_tokens
    ...
    def get_user_description(self) -> str:
        ...
        # if the existing description wasn't generated by the pr-action, just return it as-is
        if not self._is_generated_by_pr_action(description_lowercase):
            get_logger().info(f"Existing description was not generated by the pr-action")
            self.user_description = description
            return description

        # if the existing description was generated by the pr-action, but it doesn't contain a user description,
        # return nothing (empty string) because it means there is no user description
        user_description_header = "### **user description**"
        if user_description_header not in description_lowercase:
            get_logger().info(f"Existing description was generated by the pr-action, but it doesn't contain a user description")
            return ""

        # otherwise, extract the original user description from the existing pr-action description and return it
    ...
    def _possible_headers(self):
        return ("### **user description**", "### **pr type**", "### **pr description**", "### **pr labels**", "### **type**", "### **description**",
                "### **labels**", "### 🤖 generated by pr action")

    def _is_generated_by_pr_action(self, description_lowercase: str) -> bool:
        possible_headers = self._possible_headers()
