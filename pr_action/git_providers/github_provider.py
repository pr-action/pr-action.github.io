...
class GithubProvider(GitProvider):
    def __init__(self, pr_url: Optional[str] = None):
        ...
        self.base_domain_html = self.base_url_html.replace("https://", "").replace(
            "http://", ""
        )
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        ...
        try:
            ...
            for file in files:
                ...
                if self.incremental.is_incremental and self.unreviewed_files_set:
                    ...
                    patch = load_large_diff(
                        file.filename, new_file_content_str, original_file_content_str
                    )
            ...
            get_logger().error(
                f"Rate limit exceeded for GitHub API. Original message: {e}"
            )
    ...
    def publish_file_comments(self, file_comments: list) -> bool:
        try:
            ...
            if get_settings().config.verbosity_level >= 2:
                get_logger().error(
                    f"Failed to publish diffview file summary, error: {e}"
                )
            return False
    ...
    def get_repo_settings(self):
        try:
            # contents = self.repo_obj.get_contents(".pr_action.toml", ref=self.pr.head.sha).decoded_content

            # more logical to take 'pr_action.toml' from the default branch
            contents = self.repo_obj.get_contents(".pr_action.toml").decoded_content
            return contents
        except Exception:
            return ""
        ...
        if deployment_type == "user":
            try:
                ...
                raise ValueError(
                    ...
                    "https://github.com/KhulnaSoft/pr-action#method-2-run-from-source"
                ) from e
