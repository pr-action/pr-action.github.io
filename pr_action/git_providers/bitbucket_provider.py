...
class BitbucketProvider(GitProvider):
    ...
    def get_repo_settings(self):
        try:
            url = (
                ...
                f"{self.pr.destination_branch}/.pr_action.toml"
            )
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        ...
        diff_split = [
            "diff --git" + x for x in pr_patch.split("diff --git") if x.strip()
        ]
        ...
        #   --- a/pr_action/cli_pip.py
        #  +++ b/pr_action/cli_pip.py
        #   @@ -... @@"
        for i, _ in enumerate(diff_split):
            ...
            if (len(diff_split_lines) >= 6) and (
                ...
                or (
                    diff_split_lines[3].startswith("---")  # new or deleted file
                    and diff_split_lines[4].startswith("+++")
                ...
                if (
                    ...
                    get_logger().error(
                        f"Error - failed to get diff for file {_gef_filename(diffs[i])}"
                    )
                    diff_split[i] = ""
