...
from pr_action.git_providers.git_provider import IncrementalPR
from pr_action.git_providers.utils import apply_repo_settings
from pr_action.identity_providers import get_identity_provider
from pr_action.identity_providers.identity_provider import Eligibility
from pr_action.log import LoggingFormat, get_logger, setup_logger
from pr_action.servers.utils import DefaultDictWithTimeout, verify_signature
...
async def handle_new_pr_opened(
    ...
    if not (pull_request and api_url):
        ...
        return {}
    if (
        action in get_settings().github_app.handle_actions
        ...
        if ignore_pr_title_re and any(
            ...
            get_logger().info(
                f"Ignoring PR with title '{title}' due to github_app.ignore_pr_title setting"
            )
            return {}
...
def handle_closed_pr(body, event, action, log_context):
    ...
    log_context["api_url"] = api_url
    get_logger().info(
        "PR-Action statistics for closed PR",
        analytics=True,
...
async def _perform_auto_commands_github(
    commands_conf: str, agent: PRAction, body: dict, api_url: str, log_context: dict
):
    apply_repo_settings(api_url)
