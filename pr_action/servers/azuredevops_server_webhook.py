...
# The server listens for incoming webhooks from Azure DevOps Server and forwards them to the PR Action.
...
from pr_action.agent.pr_action import PRAction, command2class
    ...
    async def inner():
        try:
            with get_logger().contextualize(**log_context):
                await PRAction().handle_request(url, body)
...
async def _perform_commands_azure(commands_conf: str, agent: PRAction, api_url: str, log_context: dict):
...
async def handle_webhook(background_tasks: BackgroundTasks, request: Request):
    ...
    if data["eventType"] == "git.pullrequest.created":
        ...
        await _perform_commands_azure("pr_commands", PRAction(), pr_url, log_context)
        return
