...
class PRDescription:
    ...
    async def run(self):
        try:
            ...
            if (
                ...
                self.git_provider.is_supported("gfm_markdown")
                and get_settings().pr_description.enable_help_comment
            ):
                pr_body += "\n\n___\n\n> 💡 **PR-Action usage**:"
    ...
    async def _prepare_prediction(self, model: str) -> None:
        if (
            ...
            and "pr_action:" not in self.user_description
        ):
            get_logger().info(
        ...
        if not large_pr_handling or patches_diff:
            ...
            if patches_diff:
                ...
                if (
                    remaining_files_list
                    and "pr_files" in self.prediction
                    and "label:" in self.prediction
                    and get_settings().pr_description.mention_extra_files
                ):
                    get_logger().debug(
            ...
            for i, result in enumerate(results):
                ...
                if load_yaml(prediction_files) and prediction_files.startswith(
                    "pr_files"
                ):
...
"""
    ...
    def _prepare_pr_answer_with_markers(self) -> Tuple[str, str, str, List[dict]]:
        ...
        walkthrough_gfm = ""
        pr_file_changes = []
        if ai_walkthrough and not re.search(
            r"<!--\s*pr_action:walkthrough\s*-->", body
        ):
            try:
...
"""
            ...
            get_logger().error(
                f"Error processing pr files to markdown {self.pr_id}: {e}"
            )
            pass
        return pr_body, pr_comments
