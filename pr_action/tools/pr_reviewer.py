...
from pr_action.git_providers.git_provider import IncrementalPR, get_main_pr_language
...
class PRReviewer:
        ...
        if "code_feedback" in data:
            ...
            if get_settings().pr_reviewer.inline_code_comments:
                ...
                for suggestion in code_feedback:
                    ...
                    if hasattr(
                        self.git_provider, "generate_link_to_relevant_line_number"
                    ):
                        ...
                        if link:
                            suggestion[
                                "relevant_line"
                            ] = f"[{suggestion['relevant_line']}]({link})"
                    else:
                        pass
    ...
    def _publish_inline_code_comments(self) -> None:
        ...
        for suggestion in data.get("code_feedback", []):
            ...
            if not relevant_file or not relevant_line_in_file or not content:
                get_logger().info(
                    "Skipping inline comment with missing file/line/content"
                )
                continue
            ...
            if self.git_provider.is_supported("create_inline_comment"):
                ...
                self.git_provider.publish_inline_comment(
                    content, relevant_file, relevant_line_in_file, suggestion
                )
                ...
                if get_settings().pr_reviewer.enable_review_labels_effort:
                    ...
                    if 1 <= estimated_effort_number <= 5:  # 1, because ...
                        review_labels.append(
                            f"Review effort [1-5]: {estimated_effort_number}"
                        )
                if (
                    get_settings().pr_reviewer.enable_review_labels_security
    ...
    def auto_approve_logic(self):
        ...
        if get_settings().pr_reviewer.enable_auto_approval:
            ...
            self.git_provider.publish_comment(
                "Auto-approval option for PR-Action is disabled. "
                "You can enable it via a [configuration file](https://github.com/KhulnaSoft/pr-action/blob/main/docs/REVIEW.md#auto-approval-1)"
            )
