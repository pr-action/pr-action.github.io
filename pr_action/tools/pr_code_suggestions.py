...
from pr_action.algo.pr_processing import (
    get_pr_diff,
    get_pr_multi_diffs,
    retry_with_fallback_models,
)
from pr_action.algo.token_handler import TokenHandler
from pr_action.algo.utils import (
    ModelType,
    load_yaml,
    replace_code_tags,
...
from pr_action.git_providers import (
    AzureDevopsProvider,
    GithubProvider,
    GitLabProvider,
    get_git_provider,
...
from pr_action.servers.help import HelpMessage
from pr_action.tools.pr_description import insert_br_after_x_chars
...
class PRCodeSuggestions:
    def __init__(
        ...
        if "claude" in get_settings().config.model:
            ...
            self.pr_code_suggestions_prompt_system = (
                get_settings().pr_code_suggestions_prompt_claude.system
            )
        else:
            self.pr_code_suggestions_prompt_system = (
        ...
        self.progress += f"""\nWork in progress ...<br>\n<img src="https://khulnasoft.com/images/pr_action/dual_ball_loading-crop.gif" width=48>"""
        ...
        try:
            ...
            if not self.is_extended:
                ...
                data = await retry_with_fallback_models(
                    self._prepare_prediction_extended
                )
            if not data:
            ...
            if (
                not self.is_extended
                and get_settings().pr_code_suggestions.rank_suggestions
            ) or (
                self.is_extended
                and get_settings().pr_code_suggestions.rank_extended_suggestions
            ):
                get_logger().info("Ranking Suggestions...")
                data["code_suggestions"] = await self.rank_suggestions(
                    data["code_suggestions"]
                )
            ...
            if get_settings().config.publish_output:
                ...
                if (
                    ...
                    if (
                        ...
                        text = (
                            get_settings().pr_code_suggestions.code_suggestions_self_review_text
                        )
                        pr_body += f"\n\n- [ ]  {text}"
    ...
    def publish_persistent_comment_with_history(
        ...
        if isinstance(
            self.git_provider, AzureDevopsProvider
                ...
                for comment in prev_comments:
                    if comment.body.startswith(initial_header):
                        ...
                        if history_header.strip() not in comment.body:
                            ...
                            up_to_commit_txt = self.extract_link(
                                comment.body[:table_index]
                            )
                            prev_suggestion_table = comment.body[
                                table_index : comment.body.rfind("</table>")
                                + len("</table>")
                            ]
                            ...
                            up_to_commit_txt = self.extract_link(
                                latest_table[:table_ind]
                            )

                            latest_table = latest_table[
                                table_ind : latest_table.rfind("</table>")
                                + len("</table>")
                            ]
                ...
                if len(code_suggestions_feedback) == len(data["code_suggestions"]):
                    for i, suggestion in enumerate(data["code_suggestions"]):
                        try:
                            ...
                            suggestion["score_why"] = code_suggestions_feedback[i][
                                "why"
                            ]
                        except Exception as e:  #
                            get_logger().error(
                                ...
                                artifact={
                                    ...
                                    "code_suggestions_feedback": code_suggestions_feedback[
                                        i
                                    ],
                                },
                            )
        ...
        if self.patches_diff_list:
            ...
            if get_settings().pr_code_suggestions.parallel_calls:
                prediction_list = await asyncio.gather(
                    *[
                        self._get_prediction(model, patches_diff)
            ...
            for j, predictions in enumerate(
                ...
                if "code_suggestions" in predictions:
                    ...
                    for i, prediction in enumerate(predictions["code_suggestions"]):
                        try:
                            if (
                                get_settings().pr_code_suggestions.self_reflect_on_suggestions
                            ):
    ...
    async def rank_suggestions(self, data: List) -> List:
        ...
        try:
            ...
            environment = Environment(undefined=StrictUndefined)
            system_prompt = environment.from_string(
                get_settings().pr_sort_code_suggestions_prompt.system
            ).render(variables)
            user_prompt = environment.from_string(
                get_settings().pr_sort_code_suggestions_prompt.user
            ).render(variables)
            response, finish_reason = await self.ai_handler.chat_completion(
            ...
            for s in sort_order["Sort Order"]:
                ...
                data_sorted[importance_order - 1] = suggestion_list[
                    suggestion_number - 1
                ]
            ...
            if get_settings().pr_code_suggestions.final_clip_factor != 1:
                ...
                new_len = int(
                    0.5 + max_len * get_settings().pr_code_suggestions.final_clip_factor
                )
    ...
    def generate_summarized_suggestions(self, data: Dict) -> str:
        try:
            ...
            if get_settings().pr_code_suggestions.self_reflect_on_suggestions:
                ...
                pr_body += (
                    f"""<thead><tr><td>Category</td><td align=left>{header}</td></tr>"""
                )
            pr_body += """<tbody>"""
...
"""
    ...
    async def self_reflect_on_suggestions(
        self, suggestion_list: List, patches_diff: str, model: str
    ) -> str:
