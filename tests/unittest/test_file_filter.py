...
from pr_action.algo.file_filter import filter_ignored
from pr_action.config_loader import global_settings
...
class TestIgnoreFilter:
    def test_no_ignores(self):
        ...
        assert (
            filter_ignored(files) == files
        ), "Expected all files to be returned when no ignore patterns are given."
    ...
    def test_glob_ignores(self, monkeypatch):
        ...
        expected = [files[1], files[2]]

        filtered_files = filter_ignored(files)
        assert (
            filtered_files == expected
        ), f"Expected {[file.filename for file in expected]}, but got {[file.filename for file in filtered_files]}."
    ...
    def test_invalid_regex(self, monkeypatch):
        ...
        """
        monkeypatch.setattr(
            global_settings.ignore, "regex", ["(((||", "^file[2-4]\..*$"]
        )
