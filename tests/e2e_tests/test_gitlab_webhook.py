...
def test_e2e_run_github_app():
    ...
    try:
        ...
        for i in range(NUM_MINUTES):
            ...
            if (
                len(comments) == 2
            ):  # "changed the description" is received as the first comment
                comments_body = [comment.body for comment in comments]
                if "Work in progress" in comments_body[1]:
                    continue
                assert mr_header_body.startswith(
                    PR_HEADER_START_WITH
                ...
                logger.info(
                    f"Waiting for the MR to get all the tool results. {i + 1} minute(s) passed"
                )
        else:
            assert (
                False
            ), f"After {NUM_MINUTES} minutes, the MR did not get all the tool results"
        ...
        logger.info(
            f"Succeeded in running e2e test for GitLab app on the MR {mr.web_url}"
        )
