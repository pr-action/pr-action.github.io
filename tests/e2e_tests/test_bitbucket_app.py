...
def test_e2e_run_bitbucket_app():
    ...
    new_branch = (
        f"bitbucket_app_e2e_test-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"
    )
    ...
    try:
        ...
        repo = bitbucket_client.workspaces.get(workspace=project_key).repositories.get(
            repo_slug
        )
        ...
        url = (
            f"https://api.bitbucket.org/2.0/repositories/{project_key}/{repo_slug}/src"
        )
        ...
        requests.request(
            "POST", url, auth=HTTPBasicAuth(username, password), data=data, files=files
        )
