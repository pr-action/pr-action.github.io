...
from pr_action.log import get_logger, setup_logger
...
def test_e2e_run_github_app():
    ...
    try:
        ...
        repo.update_file(
            file.path, commit_message, NEW_FILE_CONTENT, file.sha, branch=new_branch
        )
        ...
        for i in range(NUM_MINUTES):
            ...
            if len(comments) == 2:
                ...
                assert pr_header_body.startswith(
                    PR_HEADER_START_WITH
                ), "DESCRIBE feedback is invalid"
                assert comments_body[0].startswith(
                    REVIEW_START_WITH
                ), "REVIEW feedback is invalid"
                assert re.match(
                    IMPROVE_START_WITH_REGEX_PATTERN, comments_body[1]
                ), "IMPROVE feedback is invalid"
                break
            else:
                logger.info(
                    f"Waiting for the PR to get all the tool results. {i + 1} minute(s) passed"
                )
        else:
            assert (
                False
            ), f"After {NUM_MINUTES} minutes, the PR did not get all the tool results"
        ...
        logger.info(
            f"Succeeded in running e2e test for GitHub app on the PR {pr.html_url}"
        )
